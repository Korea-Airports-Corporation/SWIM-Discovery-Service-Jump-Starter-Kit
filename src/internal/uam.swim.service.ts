...
export class UAMSWIMService {
  async myRegistry(token): Promise<any[]> {
    ...
    if(authData.me.role.name === 'uam_admin'){
      ...
      let { uamServices } = await graphQLAuthClient.request(findQuery);
      return uamServices

    }
    else if(authData.me.role.name === 'uam_publisher'){
  ...
  async updateService(token, body): Promise<any> {
    ...
    if(authData.me.role.name === 'uam_publisher'){
      ...
      let  { uamServices }  = await graphQLNoAuthClient.request(query);
      let service = uamServices[0]
      if(service){
      ...
      form.publisherId = uamServices[0].publisherId; 
      form.status = 'update'; 
      form.reject = false
      form.rejectReason = ''
      form.previousData = uamServices[0]
      ...
      await axios.put(strapiConfig.url+'uam-services/'+service['id'], JSON.stringify(form), {
        headers : headers
      }).then(resp=>{
        return resp.data
      }).catch(e => {
  ...
  async deleteService(token, identifier): Promise<any> {
    ...
    if(authData.me.role.name === 'uam_publisher'){
      ...
      await axios.delete(strapiConfig.url+'uam-services/'+identifier,{
        headers : headers
      }).then(resp=>{
        return resp.data
      }).catch(e => {
  ...
  async registerService(token, body): Promise<any> {
    ...
    if(authData.me.role.name === 'uam_publisher'){
      ...
      await axios.post(strapiConfig.url+'uam-services', JSON.stringify(form), {
        headers : headers
      }).then(resp=>{
        return resp.data
      }).catch(e => {
  ...
  async getServices(): Promise<any[]> {
      ...
      try {
        let { uamServices } = await graphQLNoAuthClient.request(query);
        let returnServices = []
        for(let i in uamServices){
          if(uamServices[i].status==='approve'){
            returnServices.push(uamServices[i]) 
          }
        }
    ...
    async getServiceForUpdate(identifier:string, token:string): Promise<any> {
      ...
      let authData = await graphQLAuthClient.request(authQuery);
      if(authData.me.role.name === 'uam_publisher'||authData.me.role.name === 'uam_admin'){
        const query = gql`
        {
          uamServices ` + filter + ` {
            serviceName,
        ...
        try {
          ...
          } else if(authData.me.role.name === 'uam_admin'){
            return returnServices[0];
          }else {
    ...
    async reject(token, identifier,action, body): Promise<any> {
      ...
      if(authData.me.role.name === 'uam_admin'){
        ...
        if(action === 'update'){
          ...
          await axios.put(strapiConfig.url+'uam-services/'+identifier, JSON.stringify(form), {
            headers : headers
          }).then(resp=>{
            return resp.data
          }).catch(e => {
